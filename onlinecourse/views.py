...
def submit(request, course_id):
    ...
    # Configure urls.py to route to this view, e.g., path('<int:course_id>/submit/', submit, name='submit')
    return render(request, 'submit.html', {'course': course, 'enrollment': enrollment})
...
def show_exam_result(request, course_id, submission_id):
    ...
    total_score = course.questions.filter(choices__in=selected_ids).aggregate(total_grade=models.Sum('grade_point'))
    
    # Check if each selected choice is a correct answer or not
    question_results = []
    for question in course.questions.all():
        is_correct = set(question.choices.filter(id__in=selected_ids).values_list('id', flat=True)) == set(selected_ids)
        question_results.append((question, is_correct))
    
    # Add the course, selected_ids, and total score to the context for rendering the HTML page
    context = {
        'course': course,
